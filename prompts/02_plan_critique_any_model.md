# 02 - Critique the Plan and Generate the Opus Revision Request - Any Model
...
Critique `FEATURE_SPEC_AND_PLAN.md` and `EXECUTION_PROMPT.md` for readiness to lock. Read original draft/interview notes when available and inspect relevant repository evidence. Write only `PLAN_CRITIQUE.md` and, when revision is needed, `OPUS_PLAN_REVISION_REQUEST.md`; do not edit code, tests, or the planning artifacts themselves.
...
Assess requirements and unresolved design decisions; original scope; concrete files/classes/functions/methods/variables and execution order; edge/error behavior; verification commands; and every Engineering Contract requirement, including backwards compatibility, API design/error translation, performance/complexity, reuse, types, dependency grounding, and Linux/Windows support.

Check each plan step and the execution prompt for exact durable documentation updates/validation or an evidence-based `Not applicable` decision. Flag execution instructions that allow divergence, omit required skills or publication rules, or authorize tests before phase `09`.

Ground findings in exact artifact passages or code evidence. Separate blocking and non-blocking issues, missing user decisions, inferences, and optional suggestions. Prefer concrete execution risks over generic style preferences or alternative future architectures.

## Required output 1: `PLAN_CRITIQUE.md`

Use this structure; keep each finding in one primary section and cross-reference it elsewhere. Give each finding a stable ID, evidence, impact, and required action. Use `None` or justified `Not applicable` for empty sections.
...
Create this complete, paste-ready Claude Opus revision prompt only when issues require revision. If none do, state that no revision pass is needed. Phase `02` is the sole producer; later verification returns here for another request.

Include a title and `## Skills`, `## Skill Handling Rule`, `## Default Planning Artifact Reduction`, `## Engineering Contract`, and `## Prompt`. Embed these skills explicitly:
...
### Revision instructions to embed

Under `## Prompt`, include role, task, context, success criteria, constraints, working method, required outputs, and final checks. Instruct Claude Opus to:

1. Read `PLAN_CRITIQUE.md`, the current planning artifacts, prior revision requests when available, draft/interview lineage, repository instructions, and relevant code. Apply valid critique within the original scope; do not implement code or tests.
2. Evaluate every critique item. Resolve valid repository-answerable items, explain rejected items, and ask before changing anything that needs my design decision or expands scope. Do not silently drop an item or speculate about unread code.
3. Update `FEATURE_SPEC_AND_PLAN.md` and `EXECUTION_PROMPT.md`, preserving all valid task detail, concrete files/symbols/order, explicit success criteria, stop rules, verification, and artifact location/metadata. Keep every step's documentation update/validation or evidence-based `Not applicable` decision.
4. Preserve the execution contract: exact plan adherence; no divergence, invented requirements, architecture changes, or unrelated refactors; no test authoring before phase `09`; full Engineering Contract and skill-loading/writing requirements. Include the execution skills and publication rules below in the revision prompt so it can repair omissions without another prompt.
5. Create `PLAN_REVISION_SUMMARY.md` using the required structure below. Check that all critique items have a disposition, no valid draft/plan detail disappeared, and the revised artifacts remain within scope and ready for verification.

### Skills for the revised `EXECUTION_PROMPT.md`

Embed these explicit links in the revision request and require them in the revised execution prompt. Preserve any additional existing execution skills; never replace the execution skill set with the critique or planning set.
...
### Execution publication and documentation rules to preserve
...
Require every implementation step to update and validate applicable durable documentation in the same change set before completion, or record an evidence-based `Not applicable` decision. Preserve the full existing implementation, regression-review, string-transformation examples, meta-content cleanup, verification, and final-response instructions; do not loosen or silently replace them.

### `PLAN_REVISION_SUMMARY.md` structure to embed
...
Final checks must verify scope, concrete implementation detail, every skill link, the full Engineering Contract, documentation checkpoints, Git/PR and artifact-exclusion rules, and the no-test-authoring boundary. Do not ask Opus to implement code.
