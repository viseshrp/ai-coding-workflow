...
If `FOLLOWUP.md` contains test-authoring items, leave them unchecked for phase `09` and report that deferral. This phase does not implement them.
...
Implement only human-approved `FOLLOWUP.md` items end-to-end, then hand off to the final test-writing phase. `FOLLOWUP.md` is the execution contract; do not add items or implement suggestions outside it.

Read `FOLLOWUP.md`, `REVIEW.md`, `WALKTHROUGH.md`, available `FEATURE_SPEC_AND_PLAN.md`, repository instructions, the current PR/branch diff against `main`, and affected code. Use review/walkthrough artifacts only as context; they do not authorize work.
...
1. Verify each approved item against the current code and its acceptance criteria. Stop and ask if it is wrong, stale, ambiguous, missing a design decision or documentation checkpoint, conflicts with instructions/code, or would cause performance, security, backwards-compatibility, or public-API problems. Report missing context or environment prerequisites; do not guess.
2. Implement the smallest correct change with no divergence, invented requirements, architecture changes, or unrelated refactors. Continue through context gathering, implementation, focused verification, and refinement without step-by-step approval unless blocked.
3. Complete the item's exact durable documentation update and validation in the same change set, or verify its evidence-based `Not applicable` decision. Code comments, commit messages, and workflow artifacts do not substitute. Check off the item only after implementation, verification, and its documentation checkpoint pass; refresh `FOLLOWUP.md` metadata.
4. Inspect the final diff for scope, compatibility, and regressions. Run focused verification for the item and applicable lint/smoke checks for every commit; report exact failed commands/logs and concrete blockers.
5. Complete Git handling below. Each commit must correspond to exactly one approved item; split a large item into sensible commits, but never mix items or unrelated cleanup. Preserve uncommitted workflow artifacts.
...
Do not create another AI review prompt or restart an AI review loop. The next phase is `09_write_focused_tests_any_model.md`: it changes test files only, creates no workflow artifact, and hands its test diff directly to a human.
...
In `Commits Created`, map every commit to its exact approved `FOLLOWUP.md` item. In `Documentation Checkpoints`, give each completed item's exact durable documentation and validation evidence or evidence-based `Not applicable` rationale. Report deferred test items and any incomplete work accurately; do not claim completion without fresh evidence.
