...
Verify that the revised planning artifacts address the prior critique and are ready for implementation. Write only `PLAN_REVISION_VERIFICATION.md`; do not edit code, tests, planning artifacts, or a revision prompt.

Read `PLAN_CRITIQUE.md`, `PLAN_REVISION_SUMMARY.md`, updated `FEATURE_SPEC_AND_PLAN.md`, updated `EXECUTION_PROMPT.md`, the prior `OPUS_PLAN_REVISION_REQUEST.md` when present, and available draft/interview notes. If no revision was required, verify the unchanged artifacts against the critique and mark the revision summary `Not applicable`; do not fabricate one.
...
For each prior concern, quote or summarize it, point to the exact revised passage or repository evidence, classify it as `Resolved`, `Partially Resolved`, `Not Resolved`, or `Invalid Concern`, and explain the disposition and remaining action. Report missing evidence and newly introduced problems.

### Documentation checkpoint verification

Verify that each implementation step in the plan and execution prompt specifies exact durable documentation, in-change-set updates and validation, or an evidence-based `Not applicable` decision. At this planning gate, completion means the documentation actions are fully specified; implementation must execute them later.
...
If any issue remains, return to `02_plan_critique_any_model.md`, the only producer of `OPUS_PLAN_REVISION_REQUEST.md`. Report a missing or incomplete request as an upstream failure; do not compensate by writing another prompt here. Only declare readiness when all valid concerns and documentation-planning checkpoints are resolved.
